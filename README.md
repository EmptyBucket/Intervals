...
Library for working with generic intervals and generic granular intervals (like a month, a quarter...)
...
```csharp
new Interval<DateTime>(new DateTime(2022, 1, 1), new DateTime(2022, 2, 15)).SplitByMonths(1)
```
...
```
This is an interval that consists of 10 granules of 1 day, but all 10 granules are shifted 1 hour forward
...
Therefore, another term month granular arises. In this type of intervals, operations are calculated based on calendar months, e.g.
```csharp
new MonthGranularInterval(new DateTime(2022, 1, 1, 1, 0, 0), new DateTime(2022, 4, 30, 1, 0, 0), 2, IntervalInclusion.Closed)
```
This is an interval that consists of 2 granules for 2 months, but all 2 granules are shifted 1 hour forward
...
// [2022-01-01, 2022-01-02) with custom granule length
